"""Run hard component majority with the promoted soft-vote tie-break."""
...
OUTPUT_DIR = PROJECT_DIR / ".runtime" / "decision-combiner-screen"
BASELINE_PATH = (
...
from decision_combiner_evaluation import evaluate_consensus_majority_vote
...
def main() -> None:
    ...
    majority = evaluate_consensus_majority_vote(
        partitioned,
        make_cross_validation(partitioned),
        baseline.xgboost,
        baseline.random_forest,
    ...
    summary = _summarise(leader, majority)
    summary.to_csv(OUTPUT_DIR / "candidate-summary.csv")
    result = {
        "majority_rows": int(majority.diagnostics["majority_rows"].sum()),
        "all_disagree_rows": int(
            majority.diagnostics["all_disagree_rows"].sum()
        ),
        "majority_accuracy": float(
            summary.loc["hard_majority_soft_tie", "mean_accuracy"]
        ),
        "change_vs_promoted_identity_vote": float(
            summary.loc["hard_majority_soft_tie", "leader_change"]
        ),
        "passes_gate": bool(
            summary.loc["hard_majority_soft_tie", "passes_gate"]
        ),
...
def _summarise(leader, majority):
    ...
    for key, candidate in {
        ...
        "hard_majority_soft_tie": majority,
    }.items():
        ...
        rows.append(
            {
                ...
                "passes_gate": key == "hard_majority_soft_tie" and (
                    change >= 0.001
