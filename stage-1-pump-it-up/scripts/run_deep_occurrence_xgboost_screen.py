"""Run categorical occurrence support in archived deep top-common XGBoost."""
...
OUTPUT_DIR = PROJECT_DIR / ".runtime" / "deep-occurrence-xgboost-screen"
OCCURRENCE_SEED = 20260822
...
def main() -> None:
    ...
    deep = joblib.load(
        ...
        / f"depth-17-seed-{OCCURRENCE_SEED}.joblib"
    )
    ...
    cache_path = OUTPUT_DIR / f"deep-occurrence-seed-{OCCURRENCE_SEED}.joblib"
    if cache_path.exists():
        occurrence = joblib.load(cache_path)
        _validate_evidence(partitioned, occurrence)
        print(f"Loaded {cache_path}.", flush=True)
    else:
        occurrence = evaluate_archived_deep_xgboost(
            partitioned,
            make_cross_validation(partitioned),
            seed=OCCURRENCE_SEED,
            preprocessor_factory=make_top_common_occurrence_preprocessor,
            representation_name="top-50 identities plus occurrence support",
            feature_policy="accepted_plus_top_50_identities_and_occurrence_support",
        )
        joblib.dump(occurrence, cache_path)
    ...
    occurrence_archive = _archive_vote(
        partitioned,
        baseline,
        occurrence,
        frequency,
        spatial,
        identity,
        "Occurrence-support deep XGBoost substitution",
    )
    candidates = {
        "archived_deep_xgboost": deep,
        "occurrence_support_deep_xgboost": occurrence,
        "existing_archive_synthesis": archive,
        "existing_deep_archive_substitution": deep_archive,
        "occurrence_support_archive_substitution": occurrence_archive,
    }
    summary = _summarise(candidates, archive)
    summary.to_csv(OUTPUT_DIR / "candidate-summary.csv")
    occurrence.diagnostics.to_csv(OUTPUT_DIR / "fold-diagnostics.csv")
    ...
    result = {
        "occurrence_fields": 29,
        "minimum_support": 5,
        "occurrence_deep_accuracy": float(
            summary.loc["occurrence_support_deep_xgboost", "mean_accuracy"]
        ),
        "archive_accuracy": float(
            summary.loc[
                "occurrence_support_archive_substitution",
                "mean_accuracy",
            ]
        ),
    ...
    print("\nDiagnostics:\n", occurrence.diagnostics.to_string(), flush=True)
...
def _validate_evidence(partitioned, *evaluations) -> None:
    for evaluation in evaluations:
        if (
            ...
            raise ValueError("Deep occurrence evidence uses different folds.")
...
def _summarise(candidates, incumbent):
    ...
    for key, candidate in candidates.items():
        ...
        eligible = key == "occurrence_support_archive_substitution"
        rows.append(
            {
                "candidate": key,
