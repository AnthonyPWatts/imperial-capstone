"""Run one faithful adapted archive frequency-forest specification."""
...
def main() -> None:
    ...
    radial = joblib.load(
        ...
        / "origin-distance-occurrence-counts.joblib"
    ).random_forest
    spatial = joblib.load(
        PROJECT_DIR
        / ".runtime"
    ...
    exact_archive = _archive_vote(
        partitioned,
        baseline,
        identity,
        trial.random_forest,
        spatial,
        "Exact-spec frequency-forest archive substitution",
    )
    candidates = {
        "existing_frequency_forest": frequency,
        "radial_frequency_forest": radial,
        "exact_archive_frequency_forest": trial.random_forest,
        "existing_archive_synthesis": archive,
        "exact_archive_substitution": exact_archive,
    }
    ...
    result = {
        "trees": ARCHIVE_FOREST_ESTIMATORS,
        ...
        "standalone_accuracy": float(
            summary.loc["exact_archive_frequency_forest", "mean_accuracy"]
        ),
        "archive_substitution_accuracy": float(
            summary.loc["exact_archive_substitution", "mean_accuracy"]
        ),
        "change_vs_existing_archive": float(
            summary.loc["exact_archive_substitution", "incumbent_change"]
        ),
        "passes_gate": bool(
            summary.loc["exact_archive_substitution", "passes_gate"]
        ),
...
def _archive_vote(partitioned, baseline, identity, frequency, spatial, name):
...
def _validate_evidence(partitioned, *evaluations) -> None:
    for evaluation in evaluations:
        if (
            ...
            raise ValueError("Exact archive evidence uses different folds.")
        ...
        rows.append(
            {
                ...
                "passes_gate": key == "exact_archive_substitution"
                and change >= 0.001
