"""Run and persist the fixed-model funder/installer feature-family screen."""
...
OUTPUT_DIR = PROJECT_DIR / ".runtime" / "high-cardinality-screen"
...
from high_cardinality_evaluation import evaluate_high_cardinality_policy
from high_cardinality_evaluation import summarise_high_cardinality_trials
from high_cardinality_features import HIGH_CARDINALITY_POLICIES
from high_cardinality_features import summarise_high_cardinality_policies
...
def main() -> None:
    ...
    selected_keys = args.policy or list(HIGH_CARDINALITY_POLICIES)
    trials = []
    for position, key in enumerate(selected_keys, start=1):
        ...
        trial = evaluate_high_cardinality_policy(
            HIGH_CARDINALITY_POLICIES[key],
            partitioned_data,
    ...
    if "baseline" in {trial.policy.key for trial in trials}:
        summary = summarise_high_cardinality_trials(trials)
        _write_summary(summary, "frozen-summary")
        print(_format_summary(summary), flush=True)
    summarise_high_cardinality_policies().to_csv(
...
def _parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--policy",
        action="append",
        choices=tuple(HIGH_CARDINALITY_POLICIES),
    ...
    parser.add_argument(
        ...
        action="append",
        choices=tuple(
            key for key in HIGH_CARDINALITY_POLICIES if key != "baseline"
        ),
        help=(
