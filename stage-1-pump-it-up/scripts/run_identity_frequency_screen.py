"""Run the fixed all-deferred-identity frequency screen."""
...
OUTPUT_DIR = PROJECT_DIR / ".runtime" / "identity-frequency-screen"
BASELINE_PATH = (
...
from identity_frequency_evaluation import evaluate_identity_frequency_trial
...
def main() -> None:
    ...
    cache_path = OUTPUT_DIR / "all-identity-frequencies.joblib"
    if cache_path.exists():
        frequency = joblib.load(cache_path)
        for evaluation in (frequency.xgboost, frequency.random_forest):
            if (
                evaluation.cross_validation_fingerprint
                != partitioned.cross_validation_fingerprint
            ):
                raise ValueError("Cached frequency trial uses different folds.")
        print(f"Loaded {cache_path}.", flush=True)
    else:
        frequency = evaluate_identity_frequency_trial(
            partitioned,
    ...
    frequency_identity_vote = _identity_vote(
        partitioned,
        frequency.xgboost,
        frequency.random_forest,
        identity,
        "All-frequency identity vote",
    )
    frequency_two_component = evaluate_weighted_soft_vote(
        partitioned,
        make_cross_validation(partitioned),
        frequency.xgboost,
        frequency.random_forest,
        weights=[0.55, 0.45],
        model_name="All-identity-frequency 55:45 vote",
    )
    summary = _summarise(
        leader,
        baseline,
        frequency,
        frequency_two_component,
        frequency_identity_vote,
    )
    ...
    result = {
        ...
        "frequency_identity_vote_accuracy": float(
            summary.loc["frequency_identity_vote", "mean_accuracy"]
        ),
        "change_vs_promoted_identity_vote": float(
            summary.loc["frequency_identity_vote", "leader_change"]
        ),
        "passes_gate": bool(
            summary.loc["frequency_identity_vote", "passes_gate"]
        ),
...
def _validate_evidence(partitioned, baseline, identity) -> None:
    for evaluation in (
        ...
        if (
            ...
            raise ValueError("Identity-frequency comparison uses different folds.")
...
def _summarise(
    ...
    for key, candidate in {
        ...
        "frequency_xgboost": frequency.xgboost,
        "accepted_random_forest": baseline.random_forest,
        "frequency_random_forest": frequency.random_forest,
        "frequency_two_component_vote": frequency_two_component,
        "frequency_identity_vote": frequency_identity_vote,
    }.items():
        ...
        rows.append(
            {
                ...
                "passes_gate": key == "frequency_identity_vote" and (
                    change >= 0.001
