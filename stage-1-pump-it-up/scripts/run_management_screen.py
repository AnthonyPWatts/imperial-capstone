"""Run and persist the fixed-model management hierarchy screen."""
...
OUTPUT_DIR = PROJECT_DIR / ".runtime" / "management-screen"
...
from management_evaluation import evaluate_management_policy
from management_evaluation import summarise_management_trials
from management_features import MANAGEMENT_POLICIES
from management_features import summarise_management_policies
...
def main() -> None:
    ...
    selected_keys = args.policy or list(MANAGEMENT_POLICIES)
    trials = []
    for position, key in enumerate(selected_keys, start=1):
        ...
        trial = evaluate_management_policy(
            MANAGEMENT_POLICIES[key],
            partitioned_data,
    ...
    if "baseline" in {trial.policy.key for trial in trials}:
        summary = summarise_management_trials(trials)
        _write_summary(summary, "frozen-summary")
        print(_format_summary(summary), flush=True)
    summarise_management_policies().to_csv(OUTPUT_DIR / "policy-register.csv")
...
def _parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--policy",
        action="append",
        choices=tuple(MANAGEMENT_POLICIES),
        help="Run one or more frozen-fold policies; default is the full screen.",
    )
    parser.add_argument(
        "--grouped-policy",
        action="append",
        choices=tuple(key for key in MANAGEMENT_POLICIES if key != "baseline"),
        help=(
