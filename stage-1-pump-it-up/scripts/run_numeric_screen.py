"""Run and persist the fixed-model numeric feature-family screen."""
...
OUTPUT_DIR = PROJECT_DIR / ".runtime" / "numeric-screen"
...
from numeric_evaluation import evaluate_numeric_policy
from numeric_evaluation import evaluate_cross_policy_hybrids
from numeric_evaluation import summarise_cross_policy_hybrids
from numeric_evaluation import summarise_numeric_trials
from numeric_features import NUMERIC_POLICIES
from numeric_features import summarise_numeric_policies
...
def main() -> None:
    ...
    selected_keys = args.policy or list(NUMERIC_POLICIES)
    trials = []
    for position, key in enumerate(selected_keys, start=1):
        ...
        trial = evaluate_numeric_policy(
            NUMERIC_POLICIES[key],
            partitioned_data,
    ...
    if "baseline" in {trial.policy.key for trial in trials}:
        summary = summarise_numeric_trials(trials)
        ...
        if "population_geographic_median" in by_key:
            hybrids = evaluate_cross_policy_hybrids(
                partitioned_data,
                make_cross_validation(partitioned_data),
                by_key["baseline"],
                by_key["population_geographic_median"],
            )
            ...
            print("Bounded component crossing:", flush=True)
            print(hybrid_summary.to_string(), flush=True)
    summarise_numeric_policies().to_csv(OUTPUT_DIR / "policy-register.csv")
...
def _parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--policy",
        action="append",
        choices=tuple(NUMERIC_POLICIES),
        help="Run one or more frozen-fold policies; default is the full screen.",
    )
    parser.add_argument(
        "--grouped-policy",
        action="append",
        choices=tuple(key for key in NUMERIC_POLICIES if key != "baseline"),
        help=(
