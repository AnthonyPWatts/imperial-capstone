"""Run the fixed ten-neighbour GPS-height imputation screen."""
...
from spatial_height_imputation_evaluation import evaluate_spatial_height_trial
...
def main() -> None:
    ...
    cache_path = OUTPUT_DIR / "ten-neighbour-height.joblib"
    if cache_path.exists():
        spatial = joblib.load(cache_path)
        for evaluation in (spatial.xgboost, spatial.random_forest):
            if (
                evaluation.cross_validation_fingerprint
                != partitioned.cross_validation_fingerprint
            ):
                raise ValueError("Cached spatial-height trial uses different folds.")
        print(f"Loaded {cache_path}.", flush=True)
    else:
        spatial = evaluate_spatial_height_trial(
            partitioned,
            make_cross_validation(partitioned),
        )
        joblib.dump(spatial, cache_path)
    ...
    full_vote = _vote(
        ...
        identity,
        "Spatial-height identity vote",
    )
    spatial_xgboost_vote = _vote(
        partitioned,
        spatial.xgboost,
        baseline.random_forest,
        identity,
        "Spatial-height XGBoost identity vote",
    )
    spatial_forest_vote = _vote(
        partitioned,
        baseline.xgboost,
        spatial.random_forest,
        identity,
        "Spatial-height Random Forest identity vote",
    )
    representation_bag_vote = evaluate_weighted_soft_vote(
        partitioned,
        make_cross_validation(partitioned),
        baseline.xgboost,
        spatial.xgboost,
    ...
    candidates = {
        ...
        "spatial_height_xgboost": spatial.xgboost,
        "accepted_random_forest": baseline.random_forest,
        "spatial_height_random_forest": spatial.random_forest,
        "spatial_height_full_vote": full_vote,
        "spatial_height_xgboost_vote": spatial_xgboost_vote,
        "spatial_height_forest_vote": spatial_forest_vote,
        "spatial_height_representation_bag_vote": representation_bag_vote,
    }
    summary = _summarise(candidates, leader)
    summary.to_csv(OUTPUT_DIR / "candidate-summary.csv")
    result = {
        "engineered_features": spatial.engineered_features,
        "transformed_features_fold_1": spatial.transformed_features_fold_1,
        "spatial_height_vote_accuracy": float(
            summary.loc[
                "spatial_height_representation_bag_vote",
                "mean_accuracy",
            ]
        ),
...
def _validate_evidence(partitioned, baseline, identity) -> None:
    for evaluation in (baseline.xgboost, baseline.random_forest, identity):
        if (
            ...
            raise ValueError("Spatial-height comparison uses different folds.")
