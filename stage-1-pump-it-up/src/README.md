...
- `catboost_identity_evaluation.py` adds all six deferred identity fields as
  ...
  also supports the four-parent and all-seven-field physical back-off
  representations plus one explicit alternative seed for bounded
...
- `probability_stack_evaluation.py` creates centred log-membership meta-features
  and evaluates one regularised cross-fitted multinomial combiner.
