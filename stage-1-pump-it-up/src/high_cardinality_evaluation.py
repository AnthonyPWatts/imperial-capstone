"""Evaluate funder and installer policies with the accepted model recipe."""
...
from high_cardinality_features import SELECTION_GATE_ACCURACY
from high_cardinality_features import SELECTION_GATE_REPAIR_RECALL
from high_cardinality_features import SELECTION_GATE_WORST_FOLD
from high_cardinality_features import HighCardinalityPolicy
...
class HighCardinalityTrial:
    """Component and blend evidence for one organisation feature policy."""
...
def evaluate_high_cardinality_policy(
    policy: HighCardinalityPolicy,
    partitioned_data: PartitionedData,
    cross_validation: _PredefinedSplit,
    *,
    evaluation_label: str = "frozen stratified folds",
) -> HighCardinalityTrial:
    """Evaluate the accepted 55:45 recipe under one organisation policy."""

    preprocessor_factory = _partial(make_high_cardinality_preprocessor, policy)
...
def summarise_high_cardinality_trials(
    trials: list[HighCardinalityTrial] | tuple[HighCardinalityTrial, ...],
) -> _pd.DataFrame:
    """Compare organisation policies with the pre-declared promotion gate."""

    if not trials:
        raise ValueError("At least one high-cardinality trial is required.")
    by_key = {trial.policy.key: trial for trial in trials}
    if len(by_key) != len(trials):
        raise ValueError("High-cardinality trial policies must be unique.")
    if "baseline" not in by_key:
        raise ValueError("The baseline high-cardinality trial is required.")
